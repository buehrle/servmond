...
def query_worker() :
    ...
    while not stop :
        out, err = Popen(["ssh", "{0}@{1}".format(settings["user"], settings["server"]), command_list], stdout=PIPE).communicate()

        for num, result in enumerate(out.splitlines()) :
            results[list(commands.keys())[num]] = str(result, encoding="utf-8")
...
server_address = '/tmp/servmond_' + os.path.splitext(os.path.basename(sys.argv[1]))[0]
